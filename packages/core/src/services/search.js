 ...
 */
export async function askQuestion(question, websiteDomain) {
    // ========================================================================
    ...
    if (!website) {
        throw new Error(`Website not found for domain: ${baseDomain}. ` +
    ...
    if (!website.gemini_store_id) {
        throw new Error(`Website "${baseDomain}" has no Gemini File Search store. ` +
    ...
    // ========================================================================
    const response = await gemini.searchWithFileSearch(website.gemini_store_id, question);
 ...
 */
export async function checkExistingContent(query, websiteDomain) {
...
Be specific and cite the actual pages from the website.`;
    ...
    const hasExistingContent = (lowerAnswer.includes('yes') || lowerAnswer.includes('found')) &&
 ...
 */
export async function searchWithFilter(question, filter) {
    ...
    const response = await gemini.searchWithFileSearch(website.gemini_store_id, question, { metadataFilter });
    return {
        answer: response.answer,
 ...
 */
export async function summarizeTopic(topic, websiteDomain) {
 ...
 */
export async function findMentions(keywords, websiteDomain) {
