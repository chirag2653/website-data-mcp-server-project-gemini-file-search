 ...
 */
export async function checkWebsiteExists(seedUrl) {
...
export async function ingestWebsite(seedUrl, displayName) {
    // ========================================================================
    ...
    if (existingWebsite) {
        ...
        if (existingJobs.length > 0) {
            ...
            if (isStuck) {
                log.info({ ingestionJobId: lastJob.id, jobAge: Math.round(jobAge / 1000) + 's' }, 'Found stuck ingestion job for existing website - attempting recovery');
                ...
                if (recoveryResult.recovered && recoveryResult.status === 'completed' && recoveryResult.result) {
                    ...
                    log.info({ ingestionJobId: lastJob.id }, 'Ingestion job is still running in FireCrawl - will continue monitoring');
                    // Job is actually still running, so we should not start a new one
                    throw new Error(`Ingestion is already in progress for this website. Job ID: ${lastJob.id}. ` +
                        `FireCrawl batch is still running. Please wait for it to complete.`);
                }
                else {
                    log.warn({ ingestionJobId: lastJob.id, recoveryStatus: recoveryResult.status, error: recoveryResult.error }, 'Recovery failed or job was already failed - will start new ingestion');
                    // Recovery failed or job was already failed, proceed with new ingestion
                }
            }
            else if (lastJob.status === 'completed') {
                log.info({ ingestionJobId: lastJob.id }, 'Last ingestion job was already completed for this website');
        ...
        log.info({ websiteId: existingWebsite.id }, 'Website exists but no active ingestion job - proceeding with new ingestion');
    }
    // ========================================================================
    ...
    if (!website) {
        ...
        if (newWebsiteJobs.length > 0) {
            ...
            const isStuck = lastJob.status === 'running' && jobAge > 60000; // 1 minute threshold
            if (isStuck) {
                log.info({ ingestionJobId: lastJob.id, jobAge: Math.round(jobAge / 1000) + 's' }, 'Found stuck ingestion job - attempting recovery');
                const recoveryResult = await recoverIngestionJob(lastJob.id);
                if (recoveryResult.recovered && recoveryResult.status === 'completed' && recoveryResult.result) {
                    log.info({ ingestionJobId: lastJob.id }, 'Successfully recovered stuck ingestion job');
                    return recoveryResult.result;
                ...
                log.info({ ingestionJobId: lastJob.id }, 'Last ingestion job was already completed - returning existing result');
    ...
    try {
        ...
        const discoveredUrls = [...new Set(filteredUrls.map(normalizeUrl))];
        ...
        const scrapeResult = await firecrawl.batchScrapeWait(batchJobId, {
            ...
            onProgress: async (completed, total) => {
                ...
                if (now - lastProgressUpdate >= 30000) {
                    ...
                    log.info({ ingestionJobId: ingestionJob.id, completed, total, percentage }, 'Updated ingestion progress in database');
                }
            },
        });
        ...
        log.info({ completed: scrapeResult.completed, total: scrapeResult.total }, 'Batch scrape complete');
 ...
 */
export async function recoverIngestionJob(ingestionJobId) {
    ...
    if (firecrawlStatus.status === 'completed' && firecrawlStatus.data) {
        ...
        log.info({ ingestionJobId, batchJobId, completed: firecrawlStatus.completed, total: firecrawlStatus.total }, 'FireCrawl batch completed - processing results');
        ...
        for (const pageData of scrapedData) {
            ...
            const url = pageData.metadata?.sourceURL;
            if (!url) {
                log.warn({ pageData }, 'Page missing sourceURL, skipping');
                continue;
            }
            if (!pageData.markdown || typeof pageData.markdown !== 'string') {
                log.warn({ url }, 'Page missing markdown, skipping');
                errors.push({ url, error: 'Missing or invalid markdown content', timestamp: now });
                continue;
            }
            const trimmedMarkdown = pageData.markdown.trim();
            if (trimmedMarkdown.length === 0) {
                log.warn({ url }, 'Empty markdown content, skipping');
                errors.push({ url, error: 'Empty content after scraping', timestamp: now });
                continue;
            }
            const contentHash = computeContentHash(pageData.markdown);
            ...
            try {
                ...
                const message = dbError instanceof Error ? dbError.message : 'Unknown error';
                log.error({ url, error: message }, 'Failed to write page to DB during recovery');
                errors.push({ url, error: message, timestamp: now });
            }
        }
 ...
 */
export async function getIngestionStatus(websiteId) {
