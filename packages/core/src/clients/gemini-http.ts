...
export async function listFileSearchStores(
  ...
  try {
    ...
    log.info(
      { storeCount: stores.length, hasNextPage: !!data.nextPageToken },
      'File Search stores listed'
    );
...
export async function listDocuments(
  ...
  try {
    ...
    log.info(
      { storeName, documentCount: documents.length, hasNextPage: !!data.nextPageToken },
      'Documents listed'
    );
...
export async function createFileSearchStore(
  ...
  try {
    ...
    return {
      ...
      displayName: (store.displayName as string) || displayName,
...
export async function uploadToFileSearchStore(
  ...
  try {
    ...
    if (operation.response) {
      log.info(
        { documentName, uploadTime: `${uploadTime}s` },
    ...
    if (operation.name) {
      ...
      const doc = completedOp.response as { documentName?: string } | undefined;
...
export async function uploadFilesInBatches(
  ...
  log.info(
    { storeName, totalItems: items.length, batchSize },
  ...
  for (let i = 0; i < items.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (item): Promise<BatchUploadResult> => {
      try {
  ...
  log.info(
    { storeName, total: items.length, successful, failed },
