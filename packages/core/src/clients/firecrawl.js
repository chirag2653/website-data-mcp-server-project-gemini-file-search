 ...
 */
export async function mapWebsite(seedUrl, options) {
    ...
    try {
        ...
        const links = (response.links ?? []).map((link) => typeof link === 'string' ? link : link.url);
        ...
        return {
            ...
            data: {
                ...
                metadata: {
                    ...
                    ...Object.fromEntries(Object.entries(metadata).filter(([key]) => !['title', 'description', 'ogImage', 'sourceURL', 'statusCode', 'language'].includes(key))),
                },
            },
        };
    }
 ...
 * @param options - Optional configuration
 */
export async function batchScrapeStart(urls, options) {
    log.info({ urlCount: urls.length }, 'Starting batch scrape');
    try {
        const response = await firecrawlRequest('/batch/scrape', {
            method: 'POST',
            body: {
                urls,
                formats: options?.formats ?? ['markdown'],
 ...
 */
export async function batchScrapeStatus(jobId) {
    try {
        const response = await firecrawlRequest(`/batch/scrape/${jobId}`, { method: 'GET' });
 ...
 */
export async function batchScrapeWait(jobId, options) {
    ...
    while (Date.now() - startTime < maxWait) {
        try {
            ...
            if (status.status === 'completed' || status.status === 'failed') {
                log.info({ jobId, status: status.status, completed: status.completed, total: status.total }, 'Batch finished');
                return status;
            }
            // Progress callback
            ...
            if (elapsed % 30000 < pollInterval) {
                ...
                log.debug({ jobId, completed: status.completed, total: status.total }, 'Batch in progress');
            }
 ...
 */
export async function batchScrapeAndWait(urls, options) {
 ...
 */
export async function batchScrapeCancel(jobId) {
    try {
